...
pub struct RefMulti<'a, K, V> {
    _guard: Arc<RwLockReadGuard<'a, HashTable<K, V>>>,
    k: *const K,
    v: *const V,
}
...
impl<'a, K: Eq + Hash, V> RefMulti<'a, K, V> {
    pub(crate) unsafe fn new(
        guard: Arc<RwLockReadGuard<'a, HashTable<K, V>>>,
        k: *const K,
        v: *const V,
    ) -> Self {
        Self {
...
pub struct RefMutMulti<'a, K, V> {
    _guard: Arc<RwLockWriteGuard<'a, HashTable<K, V>>>,
    k: *const K,
    v: *mut V,
}
...
impl<'a, K: Eq + Hash, V> RefMutMulti<'a, K, V> {
    pub(crate) unsafe fn new(
        guard: Arc<RwLockWriteGuard<'a, HashTable<K, V>>>,
        k: *const K,
        v: *mut V,
    ) -> Self {
        Self {
